    ...
    if (is_singular(['sfwd-lessons', 'sfwd-topic', 'sfwd-quiz'])) {
        ...
        jQuery(document).ready(function($) {
            ...
            function enhanceNavigationButtons() {
                ...
                // Find buttons with navigation text - cast wider net to catch all navigation buttons
                var nextButtons = $('a').filter(function() {
                    var text = $(this).text().trim();
                    var ldText = $(this).find('.ld-text').text().trim();
                    var allText = text + ' ' + ldText;
                    ...
                    return text === 'המבחן הבא' || 
                           ...
                           text === 'הנושא הבא' ||
                           ldText === 'הנושא הבא' ||
                           allText.includes('המבחן הבא') ||
                           allText.includes('השיעור הבא') ||
                           allText.includes('הנושא הבא');
                });
                ...
                nextButtons.each(function() {
                    ...
                    // Check if this is a "המבחן הבא" button that needs fixing
                    var buttonText = $button.text().trim() || $button.find('.ld-text').text().trim();
                    var needsFixing = false;
                    
                    // Fix if button leads to same page, wrong lesson, or if it's a "המבחן הבא" button
                    if (buttonUrl === currentUrl || 
                        (href && href.includes('quiz')) || 
                        buttonText === 'המבחן הבא' ||
                        (buttonUrl && buttonUrl !== currentUrl && !buttonUrl.includes('topics/'))) {
                        needsFixing = true;
                    ...
                    if (needsFixing) {
                        // Button text will be updated when AJAX response is received
                        ...
                        $button.on('click.lilac-nav', function(e) {
                            ...
                            var currentPostId = <?php echo (int)get_the_ID(); ?>;
                            var courseId = null;
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        if (response.success && response.data.next_url) {
                                            ...
                                            console.log('No next step found, using fallback');
                                            // Restore button
                ...
                backButtons.each(function() {
                    var $button = $(this);
                    console.log('Processing back button with href:', $button.attr('href'));
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.success && response.data.next_url) {
                                        ...
                                        console.log('No previous step found, using fallback');
            ...
            // Run enhancement on page load and periodically
            ...
            setTimeout(enhanceNavigationButtons, 3000);
...
function lilac_handle_get_next_step() {
            ...
            if ($parent_lesson) {
                ...
                if (!empty($lesson_topics)) {
                    ...
                    if ($current_topic_index !== false && isset($topic_ids[$current_topic_index + 1])) {
                        ...
                        // We're at the last topic of this lesson, find next lesson's first topic
                        $lessons = learndash_get_course_lessons_list($course_id);
                        $current_lesson_found = false;
                        
                        foreach ($lessons as $lesson) {
                            if ($current_lesson_found) {
                                // This is the next lesson, get its first topic
                                $next_lesson_topics = learndash_get_topic_list($lesson['post']->ID, $course_id);
                                if (!empty($next_lesson_topics)) {
                                    // Sort topics by menu order to ensure correct sequence
                                    ...
                                    // Add debug info for next lesson topics
                                    $next_lesson_debug = [];
                                    foreach ($next_lesson_topics as $topic) {
                                        $next_lesson_debug[] = [
                                            'ID' => $topic->ID,
                                            'title' => get_the_title($topic->ID),
                                            'menu_order' => $topic->menu_order,
                                            'ld_topic_order' => get_post_meta($topic->ID, '_ld_topic_order', true)
                                        ];
                                    }
                                    ...
                                    wp_send_json_success([
                                        ...
                                        'debug' => array_merge($debug_info, [
                                            'navigation_type' => 'end_of_lesson_to_next_lesson_first_topic',
                                            'current_lesson' => $parent_lesson,
                                            'next_lesson' => $lesson['post']->ID,
                                            'next_topic' => $first_topic->ID,
                                            'next_lesson_topics' => $next_lesson_debug
                                        ])
                                    ]);
                                } else {
                                    // Next lesson has no topics, go to lesson itself
                                    wp_send_json_success([
                                        'next_url' => home_url('?p=' . $lesson['post']->ID),
                                        'next_title' => get_the_title($lesson['post']->ID),
                                        'button_text' => 'לשיעור הבא',
                                        'debug' => array_merge($debug_info, [
                                            'navigation_type' => 'end_of_lesson_to_next_lesson_no_topics',
                                            'current_lesson' => $parent_lesson,
                                            'next_lesson' => $lesson['post']->ID
                                        ])
                                    ]);
    ...
    // Handle lesson-to-first-topic navigation
    if ($current_post_type === 'sfwd-lessons') {
        // Get topics for this lesson
        $lesson_topics = learndash_get_topic_list($current_post_id, $course_id);
        $debug_info['current_lesson_topics'] = [];
        ...
        if (!empty($lesson_topics)) {
            ...
            // Navigate to first topic of current lesson
            $first_topic = reset($lesson_topics);
            wp_send_json_success([
                'next_url' => home_url('?p=' . $first_topic->ID),
                'next_title' => get_the_title($first_topic->ID),
                'debug' => array_merge($debug_info, [
                    'navigation_type' => 'lesson_to_first_topic',
                    'current_lesson' => $current_post_id,
                    'first_topic' => $first_topic->ID
                ])
            ]);
        }
    }
    
    // Skip the old course steps logic - we handle navigation above for topics
    // Only use course steps as absolute fallback if topic navigation failed
...
function lilac_handle_get_previous_step() {
    ...
    // Handle topics - find previous topic in same lesson or last topic of previous lesson
    if (get_post_type($current_post_id) === 'sfwd-topic') {
        ...
        if ($parent_lesson) {
            ...
            if (!empty($lesson_topics)) {
                // Find previous topic in same lesson
                $topic_ids = array_map(function($topic) { return $topic->ID; }, $lesson_topics);
                $current_topic_index = array_search($current_post_id, $topic_ids);
                
                if ($current_topic_index !== false && $current_topic_index > 0) {
                    $previous_topic_id = $topic_ids[$current_topic_index - 1];
                    wp_send_json_success([
                        'next_url' => home_url('?p=' . $previous_topic_id),
                        'next_title' => get_the_title($previous_topic_id),
                        'debug' => array_merge($debug_info, [
                            'navigation_type' => 'previous_topic_same_lesson',
                            'current_topic_index' => $current_topic_index,
                            'previous_topic' => $previous_topic_id
                        ])
                    ]);
                } else if ($current_topic_index === 0) {
                    // We're at the first topic of this lesson, find last topic of previous lesson
                    $lessons = learndash_get_course_lessons_list($course_id);
                    $previous_lesson_found = false;
                    ...
                    foreach ($lessons as $lesson) {
                        ...
                        $previous_lesson = $lesson['post'];
                    }
                    
                    if ($previous_lesson) {
                        $previous_lesson_topics = learndash_get_topic_list($previous_lesson->ID, $course_id);
                        if (!empty($previous_lesson_topics)) {
                            $last_topic = end($previous_lesson_topics);
                            wp_send_json_success([
                                'next_url' => home_url('?p=' . $last_topic->ID),
                                'next_title' => get_the_title($last_topic->ID),
                                'debug' => array_merge($debug_info, [
                                    'navigation_type' => 'first_topic_to_previous_lesson_last_topic',
                                    'current_lesson' => $parent_lesson,
                                    'previous_lesson' => $previous_lesson->ID,
                                    'last_topic' => $last_topic->ID
                                ])
                            ]);
                        } else {
                            // Previous lesson has no topics, go to lesson itself
                            wp_send_json_success([
                                'next_url' => home_url('?p=' . $previous_lesson->ID),
                                'next_title' => get_the_title($previous_lesson->ID),
                                'debug' => array_merge($debug_info, [
                                    'navigation_type' => 'first_topic_to_previous_lesson_no_topics',
                                    'current_lesson' => $parent_lesson,
                                    'previous_lesson' => $previous_lesson->ID
                                ])
                            ]);
    ...
    // Handle quizzes - return to parent topic or lesson
    if (get_post_type($current_post_id) === 'sfwd-quiz') {
        // Try to find parent topic first
        $parent_topic = learndash_course_get_single_parent_step($course_id, $current_post_id, 'sfwd-topic');
        if ($parent_topic) {
            wp_send_json_success([
                'next_url' => home_url('?p=' . $parent_topic),
                'next_title' => get_the_title($parent_topic),
                'debug' => array_merge($debug_info, [
                    'navigation_type' => 'quiz_back_to_parent_topic',
                    'parent_topic' => $parent_topic
                ])
            ]);
        }
        
        // If no parent topic, try parent lesson
        $parent_lesson = learndash_course_get_single_parent_step($course_id, $current_post_id, 'sfwd-lessons');
        if ($parent_lesson) {
            wp_send_json_success([
                'next_url' => home_url('?p=' . $parent_lesson),
                'next_title' => get_the_title($parent_lesson),
                'debug' => array_merge($debug_info, [
                    'navigation_type' => 'quiz_back_to_parent_lesson',
    ...
    // If we get here, no previous step found
    wp_send_json_error([
        'message' => 'No previous step found',
        'debug' => $debug_info
    ]);
}
